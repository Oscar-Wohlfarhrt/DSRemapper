...
#pragma warning disable CS8618
        public DSHidDevice(string path, string description = "")
        {
            deviceInfo = new DeviceInfo(path, description);
            LoadAtributes();
        }
        public DSHidDevice(DeviceInfo info)
        {
            deviceInfo = info;
            LoadAtributes();
        }
#pragma warning restore CS8618
        ...
        public static IEnumerable<DSHidDevice> Enumerate(int vendorId, params int[] productIds)
        {
            return EnumerateDevices().Select(x => new DSHidDevice(x))
                .Where(x => x.Attributes.VendorId == vendorId && productIds.Contains(x.Attributes.ProductId));
        }
        public static IEnumerable<DSHidDevice> Enumerate(int vendorId)
        {
            return EnumerateDevices().Select(x => new DSHidDevice(x.Path, x.Description))
    ...
    public static class ByteExtensions
    {
        ...
        public static string ToUnicodeString(this byte[] buffer)
        {
